...
def main():
    ...
    keys = list(get_settings().api_keys.items())   # [(key, tenant), ...]
    passed = total = 0
    ...
    check("空 key → 401", client.get("/protected", headers={"X-API-Key": ""}).status_code == 401)
    check("错误 key → 401",
          client.get("/protected", headers={"X-API-Key": "wrong-key"}).status_code == 401)
    # key 带前后空格：当前实现是精确匹配，应被拒（提醒：要不要 strip 是设计决定）
    if keys:
        k0 = keys[0][0]
        check("带空格的正确 key → 401（精确匹配）",
              client.get("/protected", headers={"X-API-Key": f" {k0} "}).status_code == 401)

    # —— 有效凭证：每个租户的 key 都应放行、且映射到正确的 tenant ——
    if keys:
        for k, tenant in keys:
            r = client.get("/protected", headers={"X-API-Key": k})
            ok = r.status_code == 200 and r.json().get("tenant") == tenant
            check(f"正确 key（{tenant}）→ 200 且映射对", ok)
    else:
        print("  ⚠️ settings.api_keys 为空，跳过有效 key 用例——请先配 key")
