...
async def main():
    ...
    keys = list(get_settings().api_keys.keys())
    if len(keys) < 2:
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        ...
        await client.post(f"{BASE}/issues/secure", headers={"X-API-Key": key_a},
            json={"issue": "记住暗号：蓝鲸。", "user_id": "same-user", "thread_id": "same-conv"})

        # —— 正面：同租户A、同会话追问 —— 应该【记得】（证明记忆本身是工作的）
        r_same = await client.post(f"{BASE}/issues/secure", headers={"X-API-Key": key_a},
        ...
        r_other = await client.post(f"{BASE}/issues/secure", headers={"X-API-Key": key_b},
            json={"issue": "暗号是什么？", "user_id": "same-user", "thread_id": "same-conv"})
